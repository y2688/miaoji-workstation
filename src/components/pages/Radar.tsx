...
import type { RadarCategory, RadarViewMode, RadarIndustryItem, RadarTrendingItem, RadarJieqiItem } from "../../types/radar";

const ITEMS_PER_PAGE = 10;

const CATEGORY_LABELS: { key: RadarCategory; label: string; emoji: string }[] = [
  { key: "all", label: "全部", emoji: "🌐" },
  { key: "industry", label: "行业动态", emoji: "📰" },
  { key: "trending", label: "爆款追踪", emoji: "🔥" },
  { key: "jieqi", label: "节气养生", emoji: "🌸" },
];
  ...
  return (
    <div className="card-pink p-4">
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="tag tag-ribbon">热度 {formatHotCount(item.hotCount)}</span>
      </div>
      <h3 className="font-semibold text-text-dark text-sm mb-1">{item.title}</h3>
...
export default function Radar() {
  ...
  const allItems = useMemo(() => {
    ...
    // Fallback: if no server data, use built-in jieqi
    if (items.length === 0 || (items.filter((i) => i.type === "jieqi").length === 0)) {
      if (currentJieqi) {
        items.push({
          type: "jieqi",
          data: {
            ...
            tip: currentJieqi.tip,
            food: currentJieqi.food,
            recipe: currentJieqi.recipe,
            avoid: currentJieqi.avoid,
          },
          date: currentJieqi.date,
        });
      }
    ...
    <div className="space-y-4 animate-fade-in-up">
        ...
        <div className="flex gap-1">
          <button
            ...
            className={`px-3 py-1.5 rounded-btn text-xs font-medium transition-all ${
              viewMode === "card" ? "bg-pink text-white shadow-pink" : "bg-white text-text-dark border border-light-pink"
            }`}
          >
            卡片
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded-btn text-xs font-medium transition-all ${
              viewMode === "timeline" ? "bg-pink text-white shadow-pink" : "bg-white text-text-dark border border-light-pink"
            }`}
          >
            时间线
          </button>
      ...
      <div className="flex gap-2 overflow-x-auto pb-2">
        {CATEGORY_LABELS.map(({ key, label, emoji }) => (
          <button
            key={key}
            onClick={() => { setCategory(key); setVisibleCount(ITEMS_PER_PAGE); }}
            className={`tab-underline flex-shrink-0 px-4 py-2 rounded-btn text-sm font-medium transition-all ${
              category === key ? "bg-pink text-white shadow-pink active" : "bg-white text-text-dark border border-light-pink"
            }`}
          >
            {emoji} {label}
          </button>
      ...
      {!loading && !error && visibleItems.length === 0 && (
        <div className="text-center py-12 text-text-gray">
          ...
          <p className="mt-3 text-sm">暂无最新资讯，稍后再来看看</p>
        </div>
      )}
      ...
      <div className={viewMode === "timeline" ? "relative pl-6 border-l-2 border-light-pink" : "grid gap-3"}>
        {visibleItems.map((item, idx) => {
          const card =
            item.type === "industry" ? <IndustryCard item={item.data as RadarIndustryItem} /> :
